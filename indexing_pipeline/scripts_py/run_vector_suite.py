"""运行 vector retrieval suite。

这是 operator-facing CLI / 手工测试入口。

它读取 retrieval suite manifest，对每个 case：

1. 读取人工标注 dataset，拿到 query 和 candidate_paper_ids；
2. 调用 search_candidate_vector_demo 的核心函数生成 vector search output；
...
- token-hash：本地 demo vector baseline；
- openai-compatible：真实 OpenAI-compatible embedding provider，例如 bge-m3。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "读取 retrieval suite，逐 case 运行 vector search，"
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="vector suite 输出目录",
    )
    parser.add_argument(
        "--run-name",
        default="",
        help="输出文件名前缀；默认使用 provider 名称",
    )
    parser.add_argument(
        "--provider",
    ...
    parser.add_argument(
        "--embedding-cache-dir",
        default="/tmp/ai4research_vector_suite_embeddings",
        help="embedding JSONL 缓存目录",
    )
    parser.add_argument(
        "--embedding-dim",
        type=int,
        default=256,
        help="embedding 维度；openai-compatible 必须匹配 AI4RESEARCH_EMBEDDING_DIMENSION",
    )
    parser.add_argument(
...
def _search_args_for_case(
    *,
    ...
    return SimpleNamespace(
        ...
        embedding_input_max_chars=args.embedding_input_max_chars,
    ...
    *,
    ...
    return {
        ...
        "baseline": {
            "name": run_name,
            "provider": provider,
            "description": "Vector retrieval suite run.",
        },
        ...
        "notes": [
            "This summary is generated by run_vector_suite.py.",
            "Scores evaluate vector fulltext reranking inside each case's metadata candidate set.",
        ],
    }
...
def run_suite(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_name = _safe_name(args.run_name or args.provider)
    ...
    for index, suite_case in enumerate(suite_cases, start=1):
        ...
        print("=" * 100)
        print(f"[{index}/{len(suite_cases)}] vector case: {case_id}")
        print("=" * 100)
        ...
        search_output_path = output_dir / f"{case_id}_{run_name}_search_output.json"
        metrics_path = output_dir / f"{case_id}_{run_name}_metrics.json"
    ...
    summary_path = output_dir / f"retrieval_suite_v1_{run_name}_summary.json"
    _write_json(summary_path, summary)

    print("=" * 100)
    print("Vector suite summary")
    print("=" * 100)
    print(f"provider: {args.provider}")
    print(f"run_name: {run_name}")
